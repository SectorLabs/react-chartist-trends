...
class ChartistGraph extends Component {
  ...
  updateChart(config) {
    let Chartist = require('@sector-labs/chartist-trends');
    ...
    if (this.chartist) {
      ...
      this.chartist = new Chartist[type](this.chartNode, data, options, responsiveOptions);
  ...
  render() {
    ...
    return (
      <div className={`ct-chart ${className || ''}`} ref={(node) => { this.chartNode = node; }} style={style}>
         {childrenWithProps}
      </div>
    )
  }
